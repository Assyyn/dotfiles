...
# Modified to use brightnessctl by Assyyn (github.com/Assyyn)
...
function get_brightness {
    brightness=$(brightnessctl | grep -Po '[0-9]{1,3}%')
    echo ${brightness:0:-1} # strip '%'
}
...
case $1 in
    ...
    brightness=$(get_brightness)
    if [ $(( "$brightness" - "$brightness_step" )) -lt $min_brightness ]; then
